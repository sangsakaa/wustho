                        ...
                        <div class=" flex grid-cols-1 justify-end">
                            <form action="/nilaimapel" method="get" class=" flex gap-1">
                                <input type="text" name="cari" value="{{ request('cari') }}"
                                    class=" border border-green-800 text-green-800 rounded-md py-1 dark:bg-dark-bg "
                                    placeholder=" Cari ..">
                        ...
                        <table class=" w-full">
                            <thead>
                                <tr class="border bg-gray-200 dark:bg-purple-600 text-xs sm:text-sm">
                                    ...
                                    <th class=" border px-1 w-5">Kelas</th>
                            ...
                            <tbody>
                                @if ($sesikelas->count())
                                    @foreach ($sesikelas as $sesi)
                                        <tr
                                            class=" border hover:bg-gray-100  dark:hover:bg-purple-600 text-xs sm:text-sm ">
                                            ...
                                            <td class=" grid justify-items-center py-1 ">
                                                <form action="/sesikelas/{{ $sesi->id }}" method="post">
                                                    ...
                                                    <button class=" bg-red-500 text-white p-1 rounded-md"
                                                        onclick=" return confirm ('apakah anda yakin menghapus sesi ini : {{ $sesi->nama_kelas }} tgl {{$sesi->tgl}}')"><svg
