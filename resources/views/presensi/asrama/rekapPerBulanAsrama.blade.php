<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('Rekap Presensi Asrama Per Bulan') }}
        </h2>
    </x-slot>
    <div class="my-1">
        <div class="">
            <div class=" bg-white dark:bg-dark-bg overflow-hidden shadow-sm ">
                <div class="mx-2 px-2 border-gray-200 grid grid-cols-1 w-full sm:grid-cols-1  gap-2">
                    <form action="/absensikelas/rekap-per-bulan-asrama" method="get" class="w-full">
                        ...
                        <select name="kelasmi_id" id="" class=" my-1 w-full sm:w-1/5 py-1 dark:bg-dark-bg" required>
                            ...
                            <option value="{{ $kelas->id }}" {{ $kelasmi?->id === $kelas->id ? "selected" : "" }}>
                                {{ $kelas->asramasiswa_id }} {{ $kelas->periode }} {{ $kelas->ket_semester }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="  grid-cols-1 gap-2 py-1">
                        <button class=" bg-red-600 py-1 sm:w-40 rounded-sm hover:bg-purple-600 text-white px-4 " onclick="printContent('blanko')">
                            Cetak
                        </button>
                        <a href="/Exports-data" class=" text-center bg-blue-600 sm:w-40 py-1    text-white "><span class=" px-2 py-2">Export</span></a>
                    </div>
                </div>
            </div>
        </div>
