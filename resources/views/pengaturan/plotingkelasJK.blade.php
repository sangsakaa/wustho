    ...
    <div class=" flex  grid-cols-1">
      <div class=" w-full">
        <form action="/ploting-kelas-jenis-kelamin" method="get" class="flex gap-1">
          ...
          <button type="submit" class="px-2 bg-blue-500  text-white">Plotting</button>
        </form>

      </div>
      <div class=" w-full flex justify-end gap-2">
        <button class="flex text-white  bg-green-800 px-2 py-1 " onclick="printContent('div1')">
          Cetak
        </button>
    ...
    <div class=" px-4 py-1 bg-white ">
      <table class=" w-full">
        ...
        <tbody>
          ...
          @if (isset($siswas_by_kelas[0]))
          @foreach($siswas_by_kelas->groupBy('jenis_kelamin') as $jenis_kelamin => $siswas_by_jenis_kelamin)
          @if (isset($siswas_by_jenis_kelamin[0]))
          @foreach($siswas_by_jenis_kelamin->groupBy('jenis_kelasmin') as $jenis_kelasmin => $siswas_by_jenis_kelasmin)
          @if (isset($siswas_by_jenis_kelasmin[0]))
          <tr class="border-green-700 border">
            <td class="border-green-700 border" colspan="7">{{ strtoupper($kelas) }}</td>
          </tr>
          <tr class="border-green-700 border">
            <td class="border-green-700 border" colspan="7">{{ $jenis_kelamin }}</td>
          </tr>
          @foreach($siswas_by_jenis_kelasmin->chunk(35) as $siswas)
          @foreach($siswas as $data)
          <tr class="border-green-700 border">
            <td class="border-green-700 border px-1 text-center">{{ $data->kelas }}
          ...
          <tr class="border-green-700 border">
            <td class="border-green-700 border" colspan="6">{{ strtoupper($kelas) }}</td>
          </tr>
          <tr class="border-green-700 border">
