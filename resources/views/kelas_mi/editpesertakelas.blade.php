    ...
    <div class="p-4">
        <div class=" mx-auto ">
            <div class=" p-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="/pesertakelas/{{$pesertakelas->id}}" method="post">
                    @csrf
                    @method('patch')

                    <div class=" grid grid-cols-1 w-1/2 gap-2">
                        <input type="hidden" name="siswa_id" class=" py-1  " value="{{$siswaKelas->id}}" readonly>
                        <input type="text" placeholder="{{$siswaKelas->nama_siswa}}" class=" py-1" readonly>
                        ...
                        <div class=" flex gap-2">
                            <div>
                                <button class=" bg-blue-600 text-white rounded-md px-4 py-1"> Update Kelas</button>
                            </div>
                            <a href="/pesertakelas/{{$pesertakelas->kelasmi_id}}" class=" bg-red-600 text-white rounded-md px-4 py-1"> Batal</a>
                        </div>
                    </div>
                </form>
            </div>
